...
export const compareReportWithBaseline = (
  ...
  report.forEach((data: any) => {
    if (data.failed) {
      ...
      const baselineConfig = baseline.baselineConfig.find(
        (bs) => bs.url === data.url
      );
      if (!baselineConfig) {
        result[data.url] = {
        ...
        chosenCategory.forEach((category) => {
          if (baselineConfig[category]) {
            const baselineScore = Math.round(
              Number(baselineConfig[category]) * 100
            );
            if (data[category]) {
            ...
            result[data.url] = {
              ...
              [category]: {
                message:
                  "Baseline config does not exist for this metric on this particular URL",
                failed: true,
              },
            };
          }
